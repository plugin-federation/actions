...
function parseIntEnv(raw, defaultValue, name) {
  if (raw === "") return defaultValue;
  const n = Number(raw);
...
function sha256Jcs(value) {
  ...
  const hex = (0, import_node_crypto.createHash)("sha256").update(canonical, "utf8").digest("hex");
  return `sha256:${hex}`;
}
...
function projectCompositionTool(tool) {
...
function computeDigests(catalog) {
  const digests = [];
  const sortedTools = [...catalog.tools].sort(
    (a, b) => toolSortKey(a).localeCompare(toolSortKey(b))
  );
  digests.push({
    kind: "tools-array-v1",
    value: sha256Jcs({ tools: sortedTools })
  });
  if (catalog.envelope && typeof catalog.sourceId === "string" && typeof catalog.sourceVersion === "string") {
    const projected = [];
    let ok = true;
...
function setOutput(name, value) {
  ...
  import_node_fs.default.appendFileSync(out, `${name}<<${delim}
${value}
${delim}
`, "utf8");
}
...
function normalizeToolsPayload(raw) {
  ...
  if (isObject2(raw.result) && Array.isArray(raw.result.tools)) {
    ...
    return {
      tools: result.tools,
      nextCursor: typeof result.nextCursor === "string" ? result.nextCursor : void 0,
      shape: "jsonRpcResult",
      envelope: false
    };
  }
  if (Array.isArray(raw.tools)) {
    ...
    return {
      tools: raw.tools,
      nextCursor: typeof raw.nextCursor === "string" ? raw.nextCursor : void 0,
      sourceId: hasSourceId ? raw.sourceId : void 0,
      sourceVersion: hasSourceVersion ? raw.sourceVersion : void 0,
...
function resolveInputPath(userPath) {
  ...
  const absolute = import_node_path.default.isAbsolute(userPath) ? import_node_path.default.normalize(userPath) : import_node_path.default.resolve(workspace || process.cwd(), userPath);
  if (workspace) {
    const root = import_node_path.default.resolve(workspace);
    let realRoot = root;
    let realTarget = absolute;
    try {
      realRoot = import_node_fs2.default.realpathSync.native(root);
    } catch {
    }
    try {
      if (import_node_fs2.default.existsSync(absolute)) {
        realTarget = import_node_fs2.default.realpathSync.native(absolute);
      }
    } catch {
    }
    const rel = import_node_path.default.relative(realRoot, realTarget);
...
function resolveOutputPath(userPath) {
  ...
  if (import_node_path.default.isAbsolute(userPath)) {
    return import_node_path.default.normalize(userPath);
  }
...
function schemaSeverity(profile) {
  return profile === "plugin-federation" ? "error" : "warning";
}
function makeFinding(ruleId, severity, message, toolName, toolIndex, jsonPath) {
  return {
    ruleId,
...
function validateToolSchemas(tools, profile) {
  const findings = [];
...
function buildReport(args) {
  ...
  return {
    ...
    input: {
      ...
      provenanceHint: args.catalog.envelope ? "pf-tool-catalog" : "backend-or-fixture"
    },
    summary: {
      result: args.result,
      errorCount,
...
function writeReport(reportPath, report) {
  const dir = import_node_path2.default.dirname(reportPath);
  import_node_fs3.default.mkdirSync(dir, { recursive: true });
  import_node_fs3.default.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}
`, "utf8");
}
...
// src/rules.ts
var MCP_TOOL_KEYS = /* @__PURE__ */ new Set([
  "name",
  "title",
  "description",
  "inputSchema",
...
var MCP_TOOL_NAME_RE = /^[A-Za-z0-9_./-]{1,64}$/;
var SECRET_HINT = /\b(password|secret|api[_-]?key|private[_-]?key|authorization|bearer\s+[a-z0-9._~+/=-]+)\b/i;
function sev(profile, mcp, pf) {
  return profile === "mcp" ? mcp : pf;
}
function finding(ruleId, severity, message, extra = {}) {
  return {
    ruleId,
    code: ruleIdToCode(ruleId),
    severity,
...
function runRules(catalog, ctx) {
  const findings = [];
  const { profile, maxTools } = ctx;
  if (catalog.nextCursor) {
    const severity = ctx.failOnIncompleteList ? "error" : "warning";
    findings.push(
      finding(
        "PFMTL-PAGE-001",
        severity,
  ...
  if (catalog.envelope && profile === "plugin-federation") {
    if (catalog.sourceId === void 0 || typeof catalog.sourceId !== "string" || !UUID_RE.test(catalog.sourceId)) {
      findings.push(
        finding(
          "PFMTL-ENV-001",
          "error",
    ...
    if (typeof sv !== "string" || sv.length === 0 || scalarCount(sv) > 255) {
      findings.push(
        finding(
          "PFMTL-ENV-002",
          "error",
  ...
  catalog.tools.forEach((tool, index) => {
    ...
    if ("description" in tool) {
      if (typeof tool.description !== "string") {
        ...
        const s = sev(profile, "warning", "error");
        if (s !== "skip" && scalarCount(tool.description) > 4e3) {
          findings.push(
            finding(
              "PFMTL-TOOL-005",
              s,
    ...
    if ("annotations" in tool) {
      if (!isObject4(tool.annotations)) {
        ...
        for (const [k, v] of Object.entries(ann)) {
          ...
          if (["readOnlyHint", "destructiveHint", "idempotentHint", "openWorldHint"].includes(
            k
          ) && typeof v !== "boolean") {
            findings.push(
              finding(
...
function decideResult(errorCount, warningCount, failOnWarnings) {
...
async function main() {
  ...
  const toolsArrayDigest = digests.find((d) => d.kind === "tools-array-v1")?.value ?? "";
  const pfCatalogDigest = digests.find((d) => d.kind === "pf-tool-catalog-v1")?.value ?? "";
  setOutputs({
    result,
